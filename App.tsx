...
export default function App() {
 ...
 // --- Handlers ---
  ...
  const handleRiddleSubmit = () => {
    ...
    if (CUSTOM_RIDDLE_1.validAnswers.some(ans => userAnswer.includes(ans))) {
      setMission1Stage(1); // Move to Riddle 2
      setRiddleError('');
    } else {
      setRiddleError(`Nope! Here's a hint: ${CUSTOM_RIDDLE_1.hint}`);
    }
  };

  const handleRiddle2Submit = () => {
    const userAnswer = riddle2Answer.toLowerCase().trim();
    if (CUSTOM_RIDDLE_2.validAnswers.some(ans => userAnswer.includes(ans))) {
       setGameState(GameState.MISSION_2);
       setCurrentMission(2);
    } else {
       setRiddleError(`Hint: ${CUSTOM_RIDDLE_2.hint}`);
    }
  };
  ...
  const handleUnlock = async () => {
    if (passwordInput.toUpperCase().trim() === CUSTOM_FINAL_DOOR.password.toUpperCase()) {
      ...
      alert(`Wrong password! Hint: ${CUSTOM_FINAL_DOOR.question}`);
    }
  };
            ...
            <div className={`bg-white/85 backdrop-blur-md shadow-2xl rounded-3xl w-full border border-white/60 animate-float transition-all duration-500 ${gameState === GameState.MISSION_2 ? 'max-w-4xl p-4 sm:p-8' : 'max-w-md p-8'}`}>
              ...
              {gameState === GameState.UNLOCK && (
                <div className="text-center space-y-6">
                  ...
                  <p>Password Hint: {CUSTOM_FINAL_DOOR.question}</p>
